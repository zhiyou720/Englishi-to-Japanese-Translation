...
def predict_sentence(line_fr, line_en=None, display=True,
    ...
    pred_len = len(pred_ids) - 1 if pred_ids[-1] == EOS_ID else len(pred_ids)
    prec = matches / pred_len
...
def predict(s=NUM_TRAINING_SENTENCES, num=NUM_DEV_SENTENCES,
    ...
    with open(text_fname["fr"], "rb") as fr_file, open(text_fname["en"], "rb") as en_file:
        for i, (line_fr, line_en) in enumerate(zip(fr_file, en_file), start=0):
            if i >= s and i < (s + num):
                if plot:
                    plot_name = os.path.join(model_dir, "sample_{0:d}_plot.pdf".format(i + 1))
                else:
                    plot_name = None
...
def test_lam_tran():
    ...
    lines_en = ["can you do it in one day ?", "he stared at me with a satirical smile .",
...
# print(predict(s=10000, num=15, plot=True, p_filt=.5))